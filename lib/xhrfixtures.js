var xhrfixture = function (spec) {
	...
	lowercaseObjectKeys = function (obj) {
		...
		//if
		for (var i in obj) { 
			if (obj[i].constructor.toString().indexOf("Object") !== -1) { 
		...
		for (var i in patterns) { 
			if(i !== "__valid_response" && i !== "__valid_request") {
				if (patterns[i].constructor.toString().indexOf("RegExp") !== -1 
					...
					if( testobj[i].constructor.toString().indexOf("String") !== -1
				...
				} else if(patterns[i].constructor.toString().indexOf("Number") !== -1) {
					if (patterns[i] === testobj[i]) {
						found = true;
					} else {
						return false;
	...
	that.validateResponse = function (options) {
		...
		return validate(lowercaseObjectKeys(options), lowercaseObjectKeys(spec.response));
	};
	that.validateRequest = function (options) {
		options = options || spec.__valid_request; 
		return validate(lowercaseObjectKeys(options), lowercaseObjectKeys(spec.request));
	};
